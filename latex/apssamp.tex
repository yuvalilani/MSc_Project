...
\section{class BDI in one dimension}
...
\end{equation}
Since $\Gamma$ is quasidiagonal, the trace of $Im(\Gamma\tilde\Gamma)$ can be calculated with a truncated version of $\Gamma$, only using a finite region surrounding the cut.
...
\begin{figure}[h]
\centering
\includesvg[width=0.8\linewidth]{figures/1d BDI index comparison.svg}
...
\section{class D in one dimension}
\subsection{local calculation of the Majorana number of a skew symmetric unitary matrix}
We return to a general real, skew-symmetric, unitary matrix $\Gamma$, but this time without the parity flipping that was tied to TRS.
It was shown in \cite{kitaev2006} that for an infinite matrix, one can define the Majorana number of $\Gamma$ as following:
\begin{equation}
    \mathcal{M}(\Gamma)=Pf((1-2\Pi^{[0,\infty)})\Gamma(1-2\Pi^{[0,\infty)}),\Gamma)
\end{equation}
Where $Pf(A,B)$ is the \textit{Fredholm Pfaffian}. For a finite system, this definition is the equivalent to changing between a periodic and anti-periodic boundary conditions:
\begin{equation}
    \mathcal{M}(\Gamma)=\frac{Pf(\Gamma_{APBC})}{PF(\Gamma_{PBC})}
\end{equation}
From now on we will work in the infinite case for simplicity of derivation. To calculate $Pf(\tilde\Gamma. \Gamma)$, we denote $\tilde\Gamma=(1-2\Pi^{[0,\infty)})\Gamma(1-2\Pi^{[0,\infty)})$:
\begin{equation}
    \begin{split}
        Pf(\tilde\Gamma, \Gamma) &= exp \left( \frac{1}{2} Tr\left(ln(1 + K)\right) \right) \\
        K &= \Gamma\tilde\Gamma^{-1}-1=-\Gamma\tilde\Gamma-1
    \end{split} \label{eq:19}
\end{equation}
The RHS of (\ref{eq:19}) can be expanded as an infinite series in $K$, using the complete exponential Bell polynomials $B_n$:
\begin{equation}
    \begin{split}
        Pf(\tilde\Gamma, \Gamma) &= \sum_{n=0}^\infty \frac{B_n(c_1,c_2,...,c_n)}{n!} \\
        c_n&=(-1)^{n+1}\frac{n!}{2n}Tr(K^n)
    \end{split}
\end{equation}
The matrix $K$ is quasidiagonal and goes to zero far from the cut, therefore, the Majorana number $\mathcal{M}(\Gamma)$ can be calculated from a truncated version of $\Gamma$, at a finite region surrounding the cut. It is important to note here that to calculate $K$ the full $\Gamma$ is needed (the truncated version will introduce edge modes), but it is easy to show that using a double truncation one can get $\mathcal{M}(\Gamma)$ only from the truncated $\Gamma$.
...
\begin{align}
    \Gamma_D^2 + \Gamma_O^2 = -1, \; \{\Gamma_D,\Gamma_O\}=0 \label{eq:a04} \\
    [\Gamma_D,\tilde\Gamma_O]=[\Gamma_O,\tilde\Gamma_O]=0, \; \tilde\Gamma_O^2=\Gamma_O^2 \label{eq:a05}
\end{align}
...
\end{align}
Let us denote $u=\Gamma_Dv$ and from the commutation relations (\ref{eq:a04}), (\ref{eq:a05}) we have:
\begin{align}
