...
if __name__ == '__main__':
    ...
    # np.savetxt("../data/d_1d_gamma_convergence", np.array([mu, global_index, local_index[0], local_index[1], local_index[2]]))
    plt.rcParams['axes.prop_cycle'] = plt.cycler(color=plt.get_cmap('tab20').colors)
    plt.plot(mu, global_index, label="Global Index")
    for j in range(len(local_index)):
        plt.plot(mu, local_index[j], '--', label="Local Index, N="+str(j+1))
    plt.xlabel(r"$\mu /t$")
